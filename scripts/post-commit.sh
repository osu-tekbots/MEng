...
for f in $(find /nfs/ca/info/eecs_www/education/meng ! -path "*/config.php*" ! -path "*/scripts/*" ! -path "*/uploads/*" ! -path "*/.*" ! -path "*.md"); 
do
    # if [ "$f" = "." ] || [ "$f" = ".." ]; then
    #     continue
    # fi
    ...
    if [ -d "$f" ] && [ "$FILE_PERMISSIONS" != "2775" ]; then
        ...
        if [ "$yn" = "y" ]; then
            ...
            echo "Run 'chmod 775 \"$f\"' manually before committing."
        fi
    fi
done
